...
class LectureFormTest(TestCase):
    ...
    def test_create_new_object_wrong_date(self):
        ...
        self.assertEqual(errors['end_date'][0]['message'], 'End date must be greater than start date.')
...
class PointModelFormTest(TestCase):
    def setUp(self):
        self.type_of_points = TypeOfPointsModelFactory(name='Informerzy')
        self.player = PlayerModelFactory()
        self.player_2 = PlayerModelFactory()
        self.points_1 = PointModelFactory(type_of_points=self.type_of_points)
        self.points_2 = PointModelFactory(type_of_points=self.type_of_points)
    ...
    def test_wrong_code(self):
        ...
        self.assertFalse(form.is_valid())
        errors = form.errors.get_json_data()
        self.assertEqual(errors['code'][0]['message'], 'We are sorry, but the entered code is invalid! :(')
    ...
    def test_code_used_by_requesting_player(self):
        ...
        self.assertFalse(form.is_valid())
        errors = form.errors.get_json_data()
        self.assertEqual(errors['code'][0]['message'], 'We are sorry, you have already used the entered code!')
